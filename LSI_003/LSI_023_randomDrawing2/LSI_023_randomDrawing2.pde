float posX=800/2; //position
float posY=800/2;

float posXX=800/2; //previous position
float posYY=800/2;

float move=15; // steps to move

float minX = 2;
float maxX = 4;
float minY = 4;
float maxY = 6;

void setup(){ 
  size(800,800);
  background(0); 
}

void draw(){ 
 
  float randd = random(8);  // Defining probabilities
  if (randd<minX) {posX=posX+move;};
  if (randd>=minX && randd<maxX) {posX=posX-move;};
  if (randd>=minY && randd<maxY) {posY=posY-move;};
  if (randd>=maxY ) {posY=posY+move;};
 
  // Checking limits
 if(posX>width) posX=0;
 if(posX<0) posX=width;
 if(posY>height) posY=0;
 if(posY<0) posY=height;
  
  stroke(255);
 
 fill(map(posY,0,height,0,255));
 ellipse(posX, posY, move,move); 
 //Checking distance. It only draw lines, if distance is close
 if(abs(posXX-posX)<200 && abs(posYY-posY)<200 ) line(posXX, posYY, posX, posY);
 
  
  //updating previous with current position
  posXX=posX;
  posYY=posY;
  
}

void mouseReleased(){
 //background(0);  
 move = random (10,30);
 
 minX=random(0,2);
 maxX=random(minX,4);
 minY=random(maxX, 6);
 maxY=random(minY,8);
 
 println( minX+ " "+maxX+" "+minY+ " "+maxX);
 
 }